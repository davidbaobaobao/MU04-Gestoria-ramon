...
const services = [
  { title: 'Contabilidad',     desc: 'Llevanza contable completa. Sin sorpresas a final de trimestre.',                               video: '/media/service-contabilidad.mp4' },
  { title: 'Nóminas y laboral',desc: 'Elaboración de nóminas, contratos y gestiones con la Seguridad Social.',                        video: '/media/service-nominas.mp4'      },
  { title: 'Trámites',         desc: 'IVA, IRPF, IS y gestiones con organismos oficiales.',                                           video: '/media/service-tramites.mp4'     },
  { title: 'Asesoría fiscal',  desc: 'Planificación y optimización fiscal para tu empresa.',                                           video: '/media/service-fiscal.mp4'       },
]
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
              {services.map((s, i) => (
                <FadeInSection key={s.title} delay={i * 80}>
                  <ServiceVideoCard title={s.title} desc={s.desc} video={s.video} />
                </FadeInSection>
              ))}
            </div>
          </div>
        ...
        <section className="py-20 bg-[#D8F3DC]">
          <div className="max-w-[1120px] mx-auto px-6">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
              {reasons.map((r, i) => (
                <FadeInSection key={r.title} delay={i * 80}>
                  <PorqueCard title={r.title} desc={r.desc} video={r.video} />
                </FadeInSection>
              ))}
            </div>
          </div>
        ...
        <section className="py-20 bg-[#D8F3DC]">
          <div className="max-w-[1120px] mx-auto px-6 text-center">
            <FadeInSection>
              ...
              <Link
                href="/contacto"
                className="inline-flex items-center px-8 py-3 rounded-xl text-base font-semibold text-white bg-[#2D6A4F] hover:bg-[#1B4332] transition-colors duration-200 shadow-lg shadow-[#2D6A4F]/25"
              >
                Contactar ahora
              </Link>
            </FadeInSection>
